...
- `make charts` — runs `example/plot_compare.py` (goarima-AutoARIMA-vs-pmdarima), `example/plot_seasonal.py` (goarima-AutoSARIMA-vs-statsmodels-SARIMAX, AirPassengers + WineInd), and `example/plot_interval.py` (`ForecastInterval` prediction-interval lessons) at goarima's selected orders, writing charts to the gitignored `example/charts/`. The committed copies live under `docs/images/` (refresh by copying from `example/charts/`); the seasonal and interval ones are showcased in `docs/examples.md`. Needs the `example/env` venv. Not part of `make test`/CI.
...
Key files:
- `goarima.go` — `ARIMA` struct (incl. `bigP`/`bigD`/`bigQ`/`period`, `seasonalPhi`/`seasonalTheta`, cached `expandedPhi`/`expandedTheta`, `seasonalAnchors`), `NewARIMA`, `NewSARIMA`, `Fit`, `Forecast`/`forecastDiff`, getters (`Orders`/`SeasonalOrders`/`SeasonalPhi`/`SeasonalTheta`/…), `mean`/`meanSquare`/`isConstant`.
...
- `interval.go` — `ForecastInterval` and the `Forecast` result struct; `psiWeights`/`arPolynomial`/`polyMul` for the MA(∞) forecast-error variance.
