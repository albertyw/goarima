...
- `make charts` — runs `example/plot_compare.py` (goarima-AutoARIMA-vs-pmdarima) and `example/plot_seasonal.py` (goarima-AutoSARIMA-vs-statsmodels-SARIMAX, AirPassengers + WineInd) at goarima's selected orders, writing charts to the gitignored `example/charts/`. The committed copies live under `docs/images/` (refresh by copying from `example/charts/`); the seasonal ones are showcased in `docs/examples.md`. Needs the `example/env` venv. Not part of `make test`/CI.
