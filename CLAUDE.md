...
- `make charts` — runs `example/plot_compare.py`, which renders goarima-vs-pmdarima fixed-order forecast charts to `docs/images/*.png` (reads the committed `testdata/*.json` fixtures, no fitting). Needs the `example/env` venv. Not part of `make test`/CI.
...
- `example/gen_reference.py` (pmdarima reference) and `example/plot_compare.py` (trend charts, `make charts`) both read `example/data/*.csv`; the fixed orders must stay in sync across `main.go`, `compare.py`, `gen_reference.py`, and the fixtures.
- `example/compare.py` is the human eyeball comparison driver: it runs the Go example, fits `statsmodels` ARIMA at the same fixed orders as the Go `runFixed` examples, and prints the two interleaved per dataset (run via `make example`). It parses goarima's fixed-section blocks by dataset name, so the output format in `main.go` and the `FIXED` list in `compare.py` must stay in sync. statsmodels has no `auto_arima`, so the `AutoARIMA` section is goarima-only. Python deps are pinned in `example/pyproject.toml` and installed under `example/env`.
